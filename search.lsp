...
Written Spring 2014 for CSC447/547 AI class.
...
(defun search_bfs_dfs (start type)
    (do*                                                    ; note use of sequential DO*
        ...
        (setq curNode (car OPEN))
        (setq OPEN (cdr OPEN))
        (setq CLOSED (cons curNode CLOSED))
        ...
        (dolist (child (generate-successors (node-state curNode)))
            ...
            (if (and (not (member child OPEN   :test #'equal-states))
                ...
                (cond
                    ...
                    ((eq type 'bfs) (setq OPEN (append OPEN (list child))))

                    ; DFS - add to start of OPEN list (stack)
                    ((eq type 'dfs) (setq OPEN (cons child OPEN)))
...
(defun build-solution (node node-list)
    (do
        ...
        (setq node (member-state (node-parent node) node-list))

        ; add it to the path
        (setq path (cons (node-state node) path))
    )
)
